...
#Loading the dataset
col_names=['age','sex','cp','trestbps','chol','fbs','restecg','thalch','exang','oldpeak','slope','ca','thal','num']
path_data=r'datasets\heart disease\reprocessed.hungarian.data'

df=pd.read_csv(path_data, names=col_names, header=None)
df.head()

#%%Characterizing the data set
target_feature='num'
numerical_feats=['age','trestbps','chol','thalch','oldpeak',]
nominal_feats=['sex','cp','fbs','restecg','exang', 'slope','thal']
ordinal_feats=['ca']
...
def castnanvalues(df):
    df.iloc[:] = df.iloc[:].replace(-9.0,np.NaN)
    return df
...
#                             ('fx3',function3())
# ])
feat_list =['ca','thal','slope','exang', 'thalch','restecg','fbs','chol','trestbps']
#df.loc[:,feat_list]=adhoc_transf.misspellingTransformer().fit_transform(df.loc[:,feat_list])
for i in feat_list:
     print('i',i)
     df.loc[:,i]=castnanvalues((df.loc[:,i]))
my_utils.df_values(df)
...
#%%
#Features with a rate of non-values under 70%:'ca','thal','slope'
#These feature are eliminated
df=df.drop(['ca','thal','slope'],axis=1)

#%%Rename the numerical, nominal and ordinal feats
numerical_feats=['age','trestbps','chol','thalch','oldpeak']
nominal_feats=['sex','cp','fbs','restecg','exang']
ordinal_feats=[]
...
dataprep_pipe_optb=Pipeline([('data_prep',dataprep_merge_feat),
                                ('feat_sel',feature_select.Feature_Selector(strategy='wrapper_RFE') )])
...
scoring = {
    'accuracy': make_scorer(accuracy_score),
     ...
     'specificity': make_scorer(recall_score,labels=le.classes_, average='weighted'),
...
clf_fpipe_a= joblib.load(r'C:\Users\k5000751\OneDrive - Epedu O365\SeAMK\GitHub\FeatureSelection_Classifier_Pipeline\GridSearchCV_results\HD_case\Hungarian\clf_fpipe_a.pkl')
...
param_grid_fpipe_a={'clf':[dectree_clf, rndforest_clf, extratree_clf, ada_clf, xgboost_clf,gradboost_clf,voting_clf ],
                    ...
                    'data_prep__ordinal_pipe__feat_sel__strategy':['filter_cat','filter_mutinf']
                    }
...
#                      'data_prep__numeric_pipe__feat_sel__strategy':['filter_num','wrapper_RFE'],
#                      'data_prep__nominal_pipe__feat_sel__k_out_features':[1,2,3],
#                      'data_prep__nominal_pipe__feat_sel__strategy':['filter_cat','wrapper_RFE'],
...
#%% Saving the model
joblib.dump(clf_fpipe_a, r'C:\Users\k5000751\OneDrive - Epedu O365\SeAMK\GitHub\FeatureSelection_Classifier_Pipeline\GridSearchCV_results\HD_case\Hungarian\clf_fpipe_a.pkl', compress=1)
...
df_results_clf_fpipe_a.to_excel(r'C:\Users\k5000751\OneDrive - Epedu O365\SeAMK\GitHub\FeatureSelection_Classifier_Pipeline\GridSearchCV_results\HD_case\Hungarian\train_results_clf_fpipe_a.xlsx',index=True)
...
test_results_clf_fpipe_a={'clf':['clf_fpipe_a'],
                 ...
                 'f1_test':[f1_score(y_test, y_pred_clf_fpipe_a,average='weighted')],
                 'precision_test':[precision_score(y_test, y_pred_clf_fpipe_a, average='weighted')],
                 'recall_test':[recall_score(y_test, y_pred_clf_fpipe_a, average='weighted')],
                 'specificity_test':[recall_score(y_test, y_pred_clf_fpipe_a,average='weighted',labels=le.classes_)]    
    }

test_results_y_pred_clf_fpipe_a=pd.DataFrame(data=test_results_clf_fpipe_a)
test_results_y_pred_clf_fpipe_a.to_excel(r'C:\Users\k5000751\OneDrive - Epedu O365\SeAMK\GitHub\FeatureSelection_Classifier_Pipeline\GridSearchCV_results\HD_case\Hungarian\test_results_y_pred_clf_fpipe_a.xlsx',index=False)
...
#%% Load the model saved to avoid a new fitting
clf_fpipe_b= joblib.load(r'C:\Users\k5000751\OneDrive - Epedu O365\SeAMK\GitHub\FeatureSelection_Classifier_Pipeline\GridSearchCV_results\HD_case\Hungarian\clf_fpipe_b.pkl')
...
#%% Saving the model
joblib.dump(clf_fpipe_b, r'C:\Users\k5000751\OneDrive - Epedu O365\SeAMK\GitHub\FeatureSelection_Classifier_Pipeline\GridSearchCV_results\HD_case\Hungarian\clf_fpipe_b.pkl', compress=1)
...
# create an excel with the cross val resutls
df_results_clf_fpipe_b.to_excel(r'C:\Users\k5000751\OneDrive - Epedu O365\SeAMK\GitHub\FeatureSelection_Classifier_Pipeline\GridSearchCV_results\HD_case\Hungarian\train_results_clf_fpipe_b.xlsx',index=True)
...
test_results_clf_fpipe_b={'clf':['clf_fpipe_b'],
                 ...
                 'f1_test':[f1_score(y_test, y_pred_clf_fpipe_b,average='weighted')],
                 'precision_test':[precision_score(y_test, y_pred_clf_fpipe_b, average='weighted')],
                 'recall_test':[recall_score(y_test, y_pred_clf_fpipe_b, average='weighted')],
                 'specificity_test':[recall_score(y_test, y_pred_clf_fpipe_b,average='weighted',labels=le.classes_)]    
    }

test_results_y_pred_clf_fpipe_b=pd.DataFrame(data=test_results_clf_fpipe_b)
test_results_y_pred_clf_fpipe_b.to_excel(r'C:\Users\k5000751\OneDrive - Epedu O365\SeAMK\GitHub\FeatureSelection_Classifier_Pipeline\GridSearchCV_results\HD_case\Hungarian\test_results_y_pred_clf_fpipe_b.xlsx',index=False)
...
#%% Load the model saved to avoid a new fitting
clf_fpipe_c= joblib.load(r'C:\Users\k5000751\OneDrive - Epedu O365\SeAMK\GitHub\FeatureSelection_Classifier_Pipeline\GridSearchCV_results\HD_case\Hungarian\clf_fpipe_c.pkl')
...
# %%#%% Saving the model
joblib.dump(clf_fpipe_c, r'C:\Users\k5000751\OneDrive - Epedu O365\SeAMK\GitHub\FeatureSelection_Classifier_Pipeline\GridSearchCV_results\HD_case\Hungarian\clf_fpipe_c.pkl', compress=1)
...
# create an excel with the cross val resutls
df_results_clf_fpipe_c.to_excel(r'C:\Users\k5000751\OneDrive - Epedu O365\SeAMK\GitHub\FeatureSelection_Classifier_Pipeline\GridSearchCV_results\HD_case\Hungarian\train_results_clf_fpipe_c.xlsx',index=True)
...
test_results_clf_fpipe_c={'clf':['clf_fpipe_c'],
                 ...
                 'f1_test':[f1_score(y_test, y_pred_clf_fpipe_c,average='weighted')],
                 'precision_test':[precision_score(y_test, y_pred_clf_fpipe_c, average='weighted')],
                 'recall_test':[recall_score(y_test, y_pred_clf_fpipe_c, average='weighted')],
                 'specificity_test':[recall_score(y_test, y_pred_clf_fpipe_c,average='weighted',labels=le.classes_)]    
    }

test_results_y_pred_clf_fpipe_c=pd.DataFrame(data=test_results_clf_fpipe_c)
test_results_y_pred_clf_fpipe_c.to_excel(r'C:\Users\k5000751\OneDrive - Epedu O365\SeAMK\GitHub\FeatureSelection_Classifier_Pipeline\GridSearchCV_results\HD_case\Hungarian\test_results_y_pred_clf_fpipe_c.xlsx',index=False)
